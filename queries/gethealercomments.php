...
*/
						...
						if(isset($_GET['clientid'])) {
								...
								declineFunc($appointmentid, $labelid, $clientid, $conn);
						}
						else {
								die("Error!");
						}
				...
				function confirmFunc($appointmentid, $labelid, $clientid, $conn) {
						...
						if($update) {
								$sql2 = "UPDATE notification SET STATUS = 'CONFIRMED', SEEN = 'UNSEEN' WHERE NOTIF_OWNER = '$labelid' AND TYPE_ID = '$appointmentid' AND TYPE = 'appointment' AND STATUS = 'ACTIVE'";
				...
				function declineFunc($appointmentid, $labelid, $clientid, $conn) {
						$sql = "UPDATE appointment SET STATUS = 'DECLINED', DATECONFIRMED = NOW() WHERE APPOINTMENT_ID = '$appointmentid' AND CLIENT_ID = '$clientid' AND STATUS = 'ACTIVE'";
						$update = $conn->query($sql);
						if($update) {
								$sql2 = "UPDATE notification SET STATUS = 'DECLINED', SEEN = 'UNSEEN' WHERE NOTIF_OWNER = '$labelid' AND TYPE_ID = '$appointmentid' AND TYPE = 'appointment' AND STATUS = 'ACTIVE'";
								$update2 = $conn->query($sql2);
								if($update2) {
										header('Location: gethealercomments.php?accountid='.$labelid.'&q=notification');
								}
